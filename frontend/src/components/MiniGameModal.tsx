...
const GAMES: GameInfo[] = [
  { id: 'dodge', name: 'Salt Dodge', icon: '\u{1F9C2}', stat: 'AGI / REF', duration: '20s', description: 'Dodge falling salt crystals! Arrow keys or tap sides to move.' },
  { id: 'stretch', name: 'Yawn Stretch', icon: '\u{1F7E2}', stat: 'SPD', duration: '15s', description: 'Stop the slider in the green zone. Timing is everything!' },
  { id: 'lift', name: 'Projectile Lift', icon: '\u{1F4AA}', stat: 'STA', duration: '10s', description: 'Hold to fill the bar. Release before it explodes!' },
  { id: 'charm', name: 'Lucky Leaf', icon: '\u{1F340}', stat: 'LCK', duration: '5s', description: 'Pick a card and test your luck. 1x, 2x, or 3x reward!' },
  { id: 'tap', name: 'Speed Tap', icon: '\u{26A1}', stat: 'ACC', duration: '5s', description: 'Tap as fast as you can in 5 seconds!' },
]
...
function SaltDodgeGame({ onComplete }: { onComplete: (score: number) => void }) {
  ...
  const [racerX, setRacerX] = useState(50)
  const [salts, setSalts] = useState<{ id: number; x: number; y: number }[]>([])
  const [score, setScore] = useState(0)
  const [gameOver, setGameOver] = useState(false)
  const nextId = useRef(0)
  const racerXRef = useRef(50)

  useEffect(() => { racerXRef.current = racerX }, [racerX])

  const handleKey = useCallback((e: KeyboardEvent) => {
    if (e.key === 'ArrowLeft') setRacerX(prev => Math.max(5, prev - 10))
    if (e.key === 'ArrowRight') setRacerX(prev => Math.min(95, prev + 10))
  }, [])
    ...
    const t = setInterval(() => {
      setSalts(prev => {
        ...
        for (const s of prev) {
          const ny = s.y + 5
          if (ny >= 90) {
            if (Math.abs(s.x - racerXRef.current) >= 10) dodged++
          } else {
            next.push({ ...s, y: ny })
          }
        }
  ...
  return (
    <div className="relative w-full h-80 bg-brand-bg rounded-xl overflow-hidden border border-brand-border select-none"
      onTouchStart={(e) => {
        const rect = e.currentTarget.getBoundingClientRect()
        const touchX = e.touches[0].clientX - rect.left
        const pct = (touchX / rect.width) * 100
        setRacerX(Math.max(5, Math.min(95, pct)))
      }}
    >
      <div className="absolute top-2 left-2 bg-black/60 px-3 py-1 rounded-lg text-brand-primary font-bold text-sm z-10">{timeLeft}s</div>
      ...
      <div className="absolute bottom-4 text-3xl transition-all duration-75" style={{ left: `${racerX}%`, transform: 'translateX(-50%)' }}>{THEME.brand.mark}</div>
      {gameOver && (
        <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
          <p className="text-2xl font-bold text-brand-primary">Dodged {score} salts!</p>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="w-full p-6 select-none">
      <div className="flex justify-between mb-4">
        <span className="text-brand-primary font-bold">{timeLeft}s</span>
        <span className="text-white font-bold">Round {round + 1}/{maxRounds}</span>
        <span className="text-brand-primary font-bold">Hits: {score}</span>
      </div>
      <div className="relative w-full h-12 bg-brand-bg rounded-xl border border-brand-border cursor-pointer overflow-hidden" onClick={handleClick}>
...
function ShellLiftGame({ onComplete }: { onComplete: (score: number) => void }) {
  ...
  return (
    <div className="w-full p-6 text-center select-none">
      ...
      <div className="relative w-full h-16 bg-brand-bg rounded-xl border border-brand-border overflow-hidden mb-6">
      ...
      {!gameOver && (
        <button
          ...
          className={`w-full py-6 rounded-xl font-bold text-lg cursor-pointer transition-colors ${holding ? 'bg-yellow-500 text-yellow-900' : 'bg-brand-primary text-brand-bg hover:bg-brand-primary/90'}`}
        >{holding ? 'HOLDING... Release!' : 'HOLD to Lift'}</button>
      )}
      {gameOver && (
        <div className="text-center">
          {exploded ? <p className="text-red-400 font-bold text-xl">EXPLODED! Score: 0</p> : <p className="text-brand-primary font-bold text-xl">Released at {finalScore}%!</p>}
        </div>
      )}
    </div>
  )
      ...
      <div className="flex gap-4 justify-center">
        {[0, 1, 2].map(i => (
          <motion.button key={i} whileHover={picked === null ? { scale: 1.05 } : {}} whileTap={picked === null ? { scale: 0.95 } : {}} onClick={() => handlePick(i)}
            className={`w-24 h-36 rounded-xl border-2 flex flex-col items-center justify-center text-3xl font-bold cursor-pointer transition-colors ${picked === i ? 'border-brand-primary bg-brand-primary/20' : picked !== null ? 'border-brand-border bg-brand-bg opacity-60' : 'border-brand-border bg-brand-surface hover:border-brand-primary/50'}`}
          >
            {revealed ? (<><span className="text-4xl mb-1">{cardIcons[i]}</span><span className={`text-lg font-extrabold ${multipliers[i] === 3 ? 'text-yellow-400' : multipliers[i] === 2 ? 'text-brand-primary' : 'text-gray-400'}`}>{multipliers[i]}x</span></>) : picked === i ? (<motion.span animate={{ rotateY: 360 }} transition={{ duration: 0.8 }} className="text-4xl">{cardIcons[i]}</motion.span>) : (<span className="text-4xl">?</span>)}
          </motion.button>
        ))}
      </div>
      {revealed && picked !== null && (
        <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-brand-primary font-bold text-xl mt-6">{multipliers[picked]}x Multiplier!</motion.p>
      )}
    </div>
  )
}
  ...
  return (
    <div className="w-full p-6 text-center select-none">
      ...
      <motion.button whileTap={{ scale: 0.95 }} onClick={handleTap} disabled={gameOver}
        className={`w-full py-16 rounded-xl font-bold text-2xl cursor-pointer transition-colors ${gameOver ? 'bg-brand-surface border-2 border-brand-border text-gray-400' : 'bg-brand-primary text-brand-bg hover:bg-brand-primary/90 active:bg-brand-primary/80'}`}
...
// --- MiniGameModal ---
export default function MiniGameModal({ racerId, racerName, wallet, playsLeft, onClose, onGameComplete }: MiniGameModalProps) {
  ...
  async function handleGameComplete(gameType: GameType, score: number) {
    setSubmitting(true)
    try {
      const data = await api.playMiniGame(wallet, racerId, gameType, score)
        ...
        >
          ...
          <div className="flex items-center justify-between p-4 border-b border-brand-border">
            <div>
              ...
              <p className="text-gray-400 text-sm">{racerName} — {remainingPlays} plays left</p>
            </div>
          ...
          <div className="p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {GAMES.map(game => (
                  <button
                    ...
                    className="bg-brand-bg border border-brand-border rounded-xl p-4 text-left hover:border-brand-primary/50 transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    <div className="text-3xl mb-2">{game.icon}</div>
                    <p className="text-white font-bold text-sm">{game.name}</p>
                    <p className="text-brand-accent text-xs font-semibold">{game.stat} | {game.duration}</p>
            ...
            {activeGame && !result && !submitting && (
              <>
                {activeGame === 'dodge' && <SaltDodgeGame onComplete={s => handleGameComplete('dodge', s)} />}
                {activeGame === 'stretch' && <SlimeSlideGame onComplete={s => handleGameComplete('stretch', s)} />}
                {activeGame === 'lift' && <ShellLiftGame onComplete={s => handleGameComplete('lift', s)} />}
                {activeGame === 'charm' && <LuckyLeafGame onComplete={s => handleGameComplete('charm', s)} />}
                {activeGame === 'tap' && <SpeedTapGame onComplete={s => handleGameComplete('tap', s)} />}
              </>
            )}

            {submitting && (
              <div className="py-12 text-center">
                <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: 'linear' }} className="text-4xl inline-block mb-4">{THEME.brand.mark}</motion.div>
            ...
            {result && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="py-8 text-center">
                ...
                <p className="text-brand-primary font-bold text-lg mb-1">+{result.gain.toFixed(2)} {result.stat.toUpperCase()}</p>
                <p className="text-gray-400 text-sm">New {result.stat.toUpperCase()}: {result.newStatValue.toFixed(1)}</p>
                <div className="flex gap-3 justify-center mt-6">
                  <button onClick={closeGame} className="px-6 py-2.5 bg-brand-accent/20 text-brand-accent font-bold rounded-xl hover:bg-brand-accent/30 transition-colors cursor-pointer">
                    Play Another
                  </button>
                  <button onClick={handleClose} className="px-6 py-2.5 bg-brand-primary text-brand-bg font-bold rounded-xl hover:bg-brand-primary/90 transition-colors cursor-pointer">
                    Done
                  </button>
                </div>
              </motion.div>
