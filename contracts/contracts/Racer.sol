...
/**
 * @title Racer
 * @notice Racer ERC-721 with dynamic metadata — 6 stats + rarity on-chain
 */
contract Racer is ERC721, Ownable {
    ...
    mapping(uint256 => RacerData) public racerData;

    // Only RaceCore contract can mint
    ...
    constructor() ERC721("Wind-Up Rush: Showcase", "SHOWCASE") Ownable(msg.sender) {}

    /// @notice Mint a new Racer (only minter — RaceCore contract)
    function mint(
        address to,
        uint8 rarity,
        uint8 spd,
    ...
    function updateStats(
        ...
        racerData[tokenId].stats = Stats({
            spd: spd, acc: acc, sta: sta,
            agi: agi, ref: ref_, lck: lck
        });
    }

    /// @notice Get racer stats
    function getStats(uint256 tokenId) external view returns (
        ...
        RacerData storage data = racerData[tokenId];
        return (
            uint8(data.rarity),
            data.stats.spd,
            data.stats.acc,
    ...
    /// @notice Set the minter address (RaceCore contract)
